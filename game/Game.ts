...
import Crater from './entities/Crater';
...
export default class Game {
	...
	constructor(props: MultiplayerProps | SingleplayerProps) {
		const { ctx, entities, multiplayer } = props;
		this.ctx = ctx;
		...
		if (multiplayer) {
			const { socket, gameId, myTeam } = props as MultiplayerProps;
			this.socket = socket;
			this.gameId = gameId;
			this.myTeam = myTeam;
			...
			ctx.canvas.addEventListener('mousedown', (evt) => {
				if (evt.button !== 2) {
					...
					this.navListener = (evt) => {
						...
						const [x, y]: RawVector = [
							(evt.clientX - ctx.canvas.width / 2) * this.zoom + vox,
							(ctx.canvas.height / 2 - evt.clientY) * this.zoom + voy
						];
				...
				if (evt.button !== 2) {
					...
					if (this.selectBox) {
						...
						if (this.selectBox[1]) {
							...
							const point = new Vector(...this.canvasToGame([evt.clientX, ctx.canvas.height - evt.clientY]));
							this.units.forEach((unit) => {
			...
			const myUnits = this.units.filter((unit) => unit.team === this.myTeam);
			const [vpx, vpy] = myUnits.map((unit) => unit.pos).reduce(([px, py], [x, y]) => [px + x, py + y], [0, 0]);
			this.viewPos = [vpx / myUnits.length, vpy / myUnits.length];
		} else {
			// Do Singleplayer
			// Preparing event listeners
			document.addEventListener('keydown', (evt) => {
			...
			ctx.canvas.addEventListener('mouseup', (evt) => {
				if (evt.button !== 2) {
					...
					if (this.selectBox) {
						...
						if (this.selectBox[1]) {
							this.units.forEach((unit) => {
								if (unit.selectedBy(this.selectBox as [RawVector, RawVector])) {
									this.selectedUnits.push(unit);
									unit.selected = true;
								}
							});
	...
	public destroyUnit(unit: IUnit): void {
		...
		if (this.multiplayer) {
			this.socket!.emit('UNIT_DESTROYED', { gameId: this.gameId, id: unit.id });
		}
	}
